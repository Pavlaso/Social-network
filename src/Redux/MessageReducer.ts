...
const MessageReducer = (state = installState, action: ActionsType)
    :installStateType => {
    switch (action.type) {
        case 'Message/ADD_MESSAGE' : {
            return  {
                ...state,
                MesData: [...state.MesData,
                    {id: state.MesData.length + 1, message: action.newMessageBody}],
            }
        }
        default:
            return state;
...
export const actions = {
    CreatorAddMessage: (newMessageBody: string) => ({type: 'Message/ADD_MESSAGE', newMessageBody}as const )
}
